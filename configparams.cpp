    ...
    if (mParams.contains(name)) {
        ...
        if (p.type == CFG_T_DOUBLE) {
            if (p.valDouble != param) {
                qDebug() << name<< ": " << p.valDouble;
                p.valDouble = param;
                qDebug() << name<< " set: " << p.valDouble;
...
bool ConfigParams::almostEqual(double A, double B, double eps)
{
...
bool ConfigParams::saveCDefines(const QString &fileName, bool wrapIfdef)
{
    ...
    out << "// This file is autogenerated by This program\n\n";
